...
contract PiggyBank {
    ...
    // Events
    event AccountCreated(uint128 indexed accountId, address indexed owner, string name, Plan plan);
    event Deposit(address indexed sender, uint128 indexed accountId, uint256 amount, AccountType accountType);
    event Withdrawal(address indexed owner, uint128 indexed accountId, uint256 amount, uint256 interest);
    ...
    modifier onlyAccountOwner(uint128 _accountId) {
        require(accounts[_accountId].owner == msg.sender, "Only account owner can call this function");
        _;
    }
    
    modifier accountExists(uint128 _accountId) {
        require(accounts[_accountId].owner != address(0), "Account does not exist");
        _;
    }
    ...
    function createAccount(
        ...
        if (_accountType == AccountType.TOKEN) {
            require(_tokenAddress != address(0), "Token address required for token accounts");
        }
        ...
        Account memory newAccount = Account({
            ...
            tokenAddress: (_accountType == AccountType.TOKEN) ? _tokenAddress : address(0),
            plan: Plan(_plan),
            state: State.OPEN,
    ...
    function calculateInterest(uint256 _amount, uint256 _plan)
        external
        view
        returns (uint256)
    {
    ...
    function deposit(uint256 _amount, uint128 _accountId) 
        external 
        payable 
        ...
        onlyAccountOwner(_accountId)
    {
        ...
        if (accounts[_accountId].accountType == AccountType.ETHER) {
            ...
            require(msg.value == 0, "No Ether should be sent for token deposit");
        ...
        emit Deposit(msg.sender, _accountId, _amount, accounts[_accountId].accountType);
    }
    
    function withdraw(uint128 _accountId) 
        external 
        accountExists(_accountId)
        onlyAccountOwner(_accountId)
    {
    ...
    function emergencyWithdraw(uint128 _accountId) 
        external 
        accountExists(_accountId)
        onlyAccountOwner(_accountId)
    {
        ...
        if (account.accountType == AccountType.ETHER) {
            ...
            IERC20 token = IERC20(account.tokenAddress);
            require(token.transfer(msg.sender, principal), "Token transfer failed");
        }
    ...
    function getAccount(uint128 _accountId) external view returns (Account memory) {
        return accounts[_accountId];
    }
    
    function getAccountBalance(uint128 _accountId) external view returns (uint256) {
    ...
    function getTimeToMaturity(uint128 _accountId) external view returns (uint256) {
    ...
    function adminWithdrawTokenFees(address tokenAddress, uint256 amount) external onlyAdmin {
